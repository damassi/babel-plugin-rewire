...
describe("issue82", function() {
  it("Should be able to rewire React components.", function() {
    ComponentToTest.__set__("ChildComponent", function RewiredChildComponent(
      props
    ) {
    ...
    expect(result).to.be(
      '<div class="content"><span>Hello from Rewired Component</span></div>'
    );
  });

  it("Should be able to rewire Simple Functional Components.", function() {
    DefaultExportSimpleFunctionalComponent.__set__("MessageList", () => (
      <div>DIV Test</div>
    ));

    var result = ReactDOMServer.renderToStaticMarkup(
      <DefaultExportSimpleFunctionalComponent />
    );
