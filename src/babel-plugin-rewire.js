...
module.exports = function({ types: t }) {
  function isRewireable(path, variableBinding) {
    ...
    return (
      variableBinding.referencePaths !== null &&
  ...
  function doesIdentifierRepresentAValidReference(
    path,
    variableBinding,
    rewireInformation
  ) {
    let isIgnoredVariable =
      rewireInformation.ignoredIdentifiers.indexOf(path.node.name) !== -1;
    return (
      !isIgnoredVariable &&
  ...
  function getVariableNameAndBinding(path) {
    ...
    let variableBinding =
      !t.isFlow || (!t.isFlow(node) && !t.isFlow(parent))
        ? scope.getBinding(variableName)
        : undefined;
  ...
  const BodyVisitor = {
    Identifier: function(path, rewireInformation) {
      ...
      //Matches for body
      if (
        doesIdentifierRepresentAValidReference(
          path,
          variableBinding,
          rewireInformation
        )
      ) {
        let isWildCardImport =
          variableBinding.path.type === "ImportNamespaceSpecifier";
        if (
          isRewireable(path, variableBinding) &&
          contains(variableBinding.referencePaths, path)
        ) {
          if (parent.type === "UpdateExpression") {
            rewireInformation.addUpdateableVariable(variableName);
            path.parentPath.replaceWith(
              t.callExpression(rewireInformation.getUpdateOperationID(), [
                t.stringLiteral(parent.operator),
                t.stringLiteral(variableName),
                t.booleanLiteral(parent.prefix)
              ])
            );
          } else {
            rewireInformation.ensureAccessor(variableName, isWildCardImport);
            path.replaceWith(
              t.callExpression(rewireInformation.getUniversalGetterID(), [
                t.stringLiteral(variableName)
              ])
            );
          }
          ...
          if (parent.operator === "=") {
            path.parentPath.replaceWith(
              noRewire(
                t.callExpression(rewireInformation.getAssignmentOperationID(), [
                  t.stringLiteral(variableName),
                  parent.right
                ])
              )
            ...
            let baseOperator = parent.operator.substring(
              0,
              parent.operator.length - 1
            );
            path.parentPath.replaceWith(
        ...
        if (variableBinding.identifier === node) {
          rewireInformation.addTrackedIdentifier(
            variableName,
            isWildCardImport
          );
        }
    ...
    "ExportNamedDeclaration|ExportAllDeclaration": function(
      { node: { specifiers = [] } },
      rewireInformation
    ) {
      ...
      rewireInformation.hasES6DefaultExport =
        rewireInformation.hasES6DefaultExport || hasDefaultExport;
      rewireInformation.isES6Module = true;
    },

    AssignmentExpression: function(
      { node: assignmentExpression, scope: { block: { type: blockType } } },
      rewireInformation
    ) {
      rewireInformation.hasCommonJSExport =
        blockType === "Program" &&
      ...
      if (!wasProcessed(path)) {
        ...
        let declarationVisitors = {
          ClassDeclaration: function(path, rewireInformation) {
            ...
            if (
              existingClassDeclaration.id === null &&
              parent.type === "ExportDefaultDeclaration"
            ) {
              exportIdentifier = scope.generateUidIdentifier(
                "DefaultExportValue"
              );
              path.replaceWith(
                t.classDeclaration(
          ...
          FunctionDeclaration: function(path, rewireInformation) {
            ...
            if (
              existingFunctionDeclaration.id === null &&
          ...
          Identifier: function(
            { parent: { type: parentType }, node: identifier },
            rewireInformation
          ) {
        ...
        if (exportIdentifier === null) {
          exportIdentifier = noRewire(
            path.scope.generateUidIdentifier("DefaultExportValue")
          );
          path.replaceWithMultiple([
            t.variableDeclaration("let", [
              t.variableDeclarator(exportIdentifier, path.node.declaration)
            ]),
        ...
        if (!wasProcessed(path)) {
          ...
          if (rewireState.containsDependenciesToRewire()) {
            ...
            path.replaceWith(
              noRewire(
                t.Program(
                  program.body.concat(rewireState.nodesToAppendToProgramBody),
                  program.directives
                )
              )
            );
